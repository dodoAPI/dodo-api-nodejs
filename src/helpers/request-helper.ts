...
const buildRequestConfig = (configuration: Configuration): any => {
    if (!configuration?.account)
        throw new Error(Constants.messages.configuration.missingAccount);

    if (!configuration?.api)
        throw new Error(Constants.messages.configuration.missingApiName);
    ...
    if (configuration?.authType === 'BASIC') {
        if (!configuration.basicAuthData?.username)
            throw new Error(Constants.messages.configuration.missingUsername);

        if (!configuration.basicAuthData?.password)
            throw new Error(Constants.messages.configuration.missingPassword);
    ...
    if (configuration?.authType === 'KEY') {
        if (!configuration.keyAuthData?.key)
            throw new Error(Constants.messages.configuration.missingKey);
